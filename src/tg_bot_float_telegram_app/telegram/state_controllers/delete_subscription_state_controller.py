...
class DeleteSubscriptionStateController(StateController):
    async def update_all_subs_for_user(
        ...
        for index, subscription in enumerate(subscriptions):
            key = f"{subscription.weapon_name}, {subscription.skin_name}, {subscription.quality_name}, {subscription.stattrak_existence}".lower()
    ...
    async def try_get_sub_from_user_msg(
    ...
    def _found_sub_by_index(self, subs_data_dto: SubsDataDTO, index: int) -> RelationNameDTO | None:
    ...
    async def update_sub_to_delete(self, telegram_id: int, subscription: RelationNameDTO) -> None:
        telegram_id_str = str(telegram_id)
    ...
    async def get_sub_to_delete(self, telegram_id: int) -> RelationNameDTO:
        posibly_subs_to_delete = await self._get_possibly_subs_for_delete(str(telegram_id))
        return RelationNameDTO.model_validate(posibly_subs_to_delete.get("sub_to_delete"))
