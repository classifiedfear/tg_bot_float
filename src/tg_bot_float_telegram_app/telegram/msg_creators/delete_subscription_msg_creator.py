...
from tg_bot_float_telegram_app.tg_constants import DELETED_SUBSCRIPTION_MSG_TEXT
...
class DeleteSubscriptionMsgCreator:
    ...
    def create_watch_subscription_msg(self, subscriptions: List[SubscriptionDTO]) -> str:
    ...
    def create_delete_subscription_msg(self, subscription_dto: SubscriptionDTO) -> str:
        stattrak_status_msg = self._stattrak_status_msg
        if subscription_dto.stattrak:
            stattrak_status_msg = "Stattrak"
        return DELETED_SUBSCRIPTION_MSG_TEXT.format(
