...
from tg_bot_float_db_app.database.tables.relation_table import RelationsTable
...
async def create(relation: Relation, relation_table: RelationsTable = fastapi.Depends(db_dependencies.get_db_wsq_context)):
...
async def get_relation_by_id(
        ...
        relation_table: RelationsTable = fastapi.Depends(db_dependencies.get_db_wsq_context)
):
...
@relation_router.get('')
async def get_relations(relation_table: RelationsTable = fastapi.Depends(db_dependencies.get_db_wsq_context)):
