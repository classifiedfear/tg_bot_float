...
class QualityRouter:
    ...
    def _init_routes(self):
        ...
        self._router.add_api_route(
            "/name/{quality_name}", self._update_quality_by_name, response_model=None, methods=["PUT"]
        )
        self._router.add_api_route(
        ...
        async with service_factory:
            ...
            try:
                ...
                return MsgResponseDTO(status=False, msg=ITEM_EXIST_MSG.format(
                    item='Quality',
                    identifier='name',
                    item_identifier=quality_dto.name))
    ...
    async def _get_quality_by_name(
        ...
        async with service_factory:
            ...
            return MsgResponseDTO(status=False, msg=ITEM_NOT_EXIST_MSG.format(
                    item='Quality',
                    identifier='name',
                    item_identifier=quality_name)
                )

    async def _update_quality_by_name(
            self, service_factory: BOT_DB_SERVICE_FACTORY, quality_name: str, quality_dto: QualityDTO,
    ) -> MsgResponseDTO | QualityModel:
        async with service_factory:
            quality_service = service_factory.get_quality_service()
            try:
